...
<code><>... $storedVariableName ...<></code> works. However, <code><>...${storedVariableName}...<></code> (as per [ClassicSelenese](ClassicSelenese) > [Stored variables](ClassicSelenese#stored-variables)) doesn't work. Side note: We don't want this combination anyway, because `${storedVariableName}` works through string substitution. It could cause unexpected Javascript errors. E.g. `${variableName}` could be a number when developing the script. However, later it could become a non-numeric string. If there were no quotes/apostrophes around it, an the Javascript expression would expect a number, it would fail. Also, it would need extra handling of strings containing apostrophes/quotes. Indeed, `${storedVariableName}` works in any prefix/suffix of <code><>...<></code> (as per standard Selenese).

## Passing (sub)string &lt;&gt; verbatim
To pass actual text `<>` to Selenium, generate it from an expression. For example, pass `<>'<' + '>'<>`. See [SelBlocksGlobal/selenese-tests/misc_case.html](http://htmlpreview.github.io/?https://github.com/SeLite/SelBlocksGlobal/blob/master/selenese-tests/misc_case.html).
